...
def encrypt_this(words):
    ...
    for word in word_list:
        # Transform word to be ASCII code of first letter and second letter transposed with last letter
        # print(f"Encrypting word:{word}")
        ...
        # if len(word) > 2:
        #     encrypted_word = str(ord(word[0])) + word[len(word)-1] + word[2:len(word)-1] + word[1]
        # elif len(word) == 2:
        #     encrypted_word = str(ord(word[0])) + word[len(word) - 1]

        # print(f"Encryption is:{encrypted_word}")
